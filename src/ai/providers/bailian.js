 ...
 */
class BailianProvider extends BaseAIProvider {
  constructor(config) {
    super(config);
    // 支持套餐专属 Base URL
    this.baseUrl = config.baseUrl || 'https://coding.dashscope.aliyuncs.com/v1';
  ...
  async generateTags(document, existingTags = []) {
    ...
    console.log('🌐 Sending request to Bailian API...');
  ...
  parseResponse(data) {
    try {
      ...
      throw new Error(`Failed to parse Bailian response: ${error.message}`);
    }
  }
